    ...
    try:
        ...
        if mode == "merge":
            ...
            if bgm_path and has_sound:
                args += ["-filter_complex", "[1:a]volume=0.25[b];[0:a][b]amix=inputs=2:duration=first[a]", "-map", "0:v", "-map", "[a]"]
            elif bgm_path:
                ...
                args += ["-an"]
            args += ["-vf", f"{vf_base},format=yuv420p", "-c:v", "libx264", "-preset", "fast", "-r", "30", "-c:a", "aac", out_path]
            ...
            if bg_file and bg_file.filename:
                ...
                args = ["-stream_loop", "-1", "-t", str(total), "-i", bg_path] if is_video else ["-loop", "1", "-t", str(total), "-i", bg_path]
            else:
                args = ["-f", "lavfi", "-i", f"color=c=0x1a1a2e:s=1080x1920:r=30:d={total}"]
            if bgm_path:
                args += ["-i", bgm_path, "-map", "0:v", "-map", "1:a", "-shortest"]
            args += ["-vf", f"{vf_base},{draw},format=yuv420p", "-c:v", "libx264", "-preset", "fast", "-r", "30", "-c:a", "aac", out_path]
